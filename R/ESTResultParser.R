...
#'   \item{\code{parseReport(fileName = "analysis_report.csv")}}{This method 
#'   parses the analysis report file (analysis_report.csv).}
#'   \item{\code{parseAR(fileName = "ar_results.csv")}}{This method parses the 
...
ResultParser <- R6::R6Class(classname = "ResultParser",
                            public = list(
                              ...
                              parseRequestFile = function(path = "01_RequestFile.csv") {
                                parseReturn <- private$parseFile(path, "requestFile")
                                if (parseReturn) {
                                  # add groups
                                  groups <- unique(self$requestFile$V5)
                                }
                                parseReturn
                              },
                              ...
                              parseAAR = function(fileName = "aar_results.csv", groups = NULL) {
                                ...
                                # parse AAR values
                                aarResults <- data.table::fread(fileName)
